...
class TestParser < Test::Unit::TestCase
  ...
  def test_extract_seq
    raw_ping = "64 bytes from gigamed1.red.uv.es (147.156.241.4): icmp_seq=1 ttl=58 time=9.38 ms"
    assert_equal "1", Parser.read_seq(raw_ping)
  end
  
  def test_read_time_from_a_file
    file = File.expand_path(File.dirname(__FILE__) + "/fixtures/ping.txt")
    assert_equal [ 9.38, 7.07, 9.74, 8.51, 3.77, 9.86, 8.73, 8.22, 107 ], Parser.read_file(file)[0]
  end
  
  def test_read_seq_from_a_file
    file = File.expand_path(File.dirname(__FILE__) + "/fixtures/ping.txt")
    assert_equal [1, 2, 3, 4, 5, 6, 7, 8, 9], Parser.read_file(file)[1]
  end
